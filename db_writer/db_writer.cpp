...
db_writer::db_writer(string url, writer *writer_ptr) {
    ...
    if (info.counter) {
        info.counter++;
        logger_ptr->info("connect to existed db", url);
    } else {
    ...
    db_handle = static_cast<void *>(info.db_ptr);
    stmt_handle = static_cast<void *>(info.stmt_ptr);
    logger_ptr = info.logger_ptr;
    db_mutex = &info.db_mutex;
}
...
db_writer::~db_writer() {
    ...
    if (info.counter == 0) {
        if (SQLITE_OK != sqlite3_finalize(info.stmt_ptr)) {
            logger_ptr->error("finalize stmt failed", sqlite3_errmsg(info.db_ptr));
        }
        if (SQLITE_OK != sqlite3_close(info.db_ptr)) {
            logger_ptr->error("close db failed", sqlite3_errmsg(info.db_ptr));
        }
        logger_ptr->info("shut db", url);
        delete info.logger_ptr;
        db_map.erase(url);
    } else {
        logger_ptr->info("shut db_writer", url);
    }
}

void db_writer::write(level l, string module_name, string comment, string data) {
    sqlite3 *db_ptr = static_cast<sqlite3 *>(db_handle);
    sqlite3_stmt *stmt_ptr = static_cast<sqlite3_stmt *>(stmt_handle);
    int ret;
    std::lock_guard<std::mutex> lock{*db_mutex};
    do {
        ret = sqlite3_bind_int64(stmt_ptr, 1, get_nano_timestamp());
        if (ret != SQLITE_OK) {
            logger_ptr->error("bind ts failed", sqlite3_errmsg(db_ptr));
            break;
        }
        ret = sqlite3_bind_text(stmt_ptr, 2, module_name.c_str(), module_name.length() + 1, SQLITE_STATIC);
        if (ret != SQLITE_OK) {
            logger_ptr->error("bind module_name failed", sqlite3_errmsg(db_ptr));
            break;
        }
        ret = sqlite3_bind_text(stmt_ptr, 3, comment.c_str(), comment.length() + 1, SQLITE_STATIC);
        if (ret != SQLITE_OK) {
            logger_ptr->error("bind comment failed", sqlite3_errmsg(db_ptr));
            break;
        }
        ret = sqlite3_bind_text(stmt_ptr, 4, data.c_str(), data.length() + 1, SQLITE_STATIC);
        if (ret != SQLITE_OK) {
            logger_ptr->error("bind data failed", sqlite3_errmsg(db_ptr));
            break;
        }
        ret = sqlite3_step(stmt_ptr);
        if (ret != SQLITE_DONE) {
            logger_ptr->error("step stmt failed", sqlite3_errmsg(db_ptr));
        }
        ret = sqlite3_reset(stmt_ptr);
        if (ret != SQLITE_OK) {
